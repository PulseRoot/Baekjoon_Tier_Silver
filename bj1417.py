...
for i in range(0, hubo):
   Pyo = int(input()) 
   list.append(Pyo)

while(1):                      #계속 반복한다
    for i in range(1, hubo):   #1번 후보인 다솜이 보다 더 높은 투표수를 받은 후보가 없을 때까지  
        if list[0] <= list[i]:     #만약 더 높은 후보가 있다면 
            list[0] = list[0] + 1  #한표를 빼앗아 온다
            list[i] = list[i] - 1 
            buy = buy + 1
    if list[0] == max(list):   #만약에 리스트에서 가장 큰 값을 지닌 것이 다솜이라면 탈출한다.
        break    
